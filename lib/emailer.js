...
async function emailer(document, tenantId, term) {
    ...
    const results = [];
    const email_data = {document, tenantId, term, ...data};
    for (let email_address of email_addresses) {
        ...
        if (!email_address) {
            ...
            const email = {
                ...emailRecipients(email_address),
                ...await emailAttachment(document, email_data),
                ...await emailContent(document, email_data)
            };
            logger.debug(email);
